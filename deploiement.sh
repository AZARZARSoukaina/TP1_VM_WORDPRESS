...
##############################################################################
USER="soukaina_az59" # Remplacez "" par le nomutilisateur

# Remplacez "VOTRE_PROJET" par l'ID de votre projet GCP
PROJET="cursusm2i-soukaina"
...
if [ ! -d "Terraform" ]; then
    #git clone 
    #cd 
    echo "dossier Terraform n'existe pas"
else
    cd Terraform
fi
terraform init
...
if grep -q "wordpress-vm" nom_des_instances.txt; then
    ansible-playbook playbook_wordpress.yml -i "./gcp_compute.yml"
fi
if grep -q "database-vm" nom_des_instances.txt; then
    ansible-playbook playbook_database.yml -i "./gcp_compute.yml"
fi
# 7- Vérification que l'application fonctionne
wordpress_ip="$(gcloud compute instances describe wordpress-vm --project $PROJET --zone $ZONE --format='get(networkInterfaces[0].accessConfigs[0].natIP)')"
echo $wordpress_ip
